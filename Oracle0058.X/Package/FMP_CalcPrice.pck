...
ON A.YY=B.YY) o,(select selid,tsid,version,' ||
  ...
  BEGIN
    ...
    vSQL := 'INSERT /*+ append parallel */ INTO ' || vTempTableInner ||
            ' with tab as(
...
where a.selid=b.selid  and a.version=b.version and a.yymm=b.yymm
  ...
  BEGIN
    ...
    v_strSql := 'INSERT /*+ append */ INTO ' || vTempTableInner || '
 with tab as (
...
pivot (max(price) for mm in (1 as t1, 2 as t2 ,3 as t3 ,4 as t4, 5 as t5 ,6 as t6 , 7 as t7 , 8 as t8
  ...
  PROCEDURE FMSP_CalcPrice(pIn_nChronology         IN NUMBER, --1 Month ,2 Week,3 Day
                           pIn_nHistValDBID        IN NUMBER,
                           pIn_nHistQtyDBID        IN NUMBER,
                           pIn_nPriceDBID          IN NUMBER,
  ...
  BEGIN
    ...
    IF v_oOptions.bSel OR pIn_vOptions IS NULL THEN
      ...
      using (select m.pvtid
               from don_m m
              where m.tsid = ' || pIn_nPriceDBID || '
                and m.yy between ' ||
                    substr(pIn_nBeginDate, 1, 4) || ' and
                    ' || substr(pIn_nEndCalcOfDate, 1, 4) || '
             minus
             select m.pvtid
               from don_m m
              where m.tsid in (' || pIn_nHistValDBID || ', ' ||
                    pIn_nHistQtyDBID || ')
                and m.yy between ' ||
                    substr(pIn_nBeginDate, 1, 4) || ' and
      ...
      when matched then
        update
         ...
         where d.tsid = ' || pIn_nPriceDBID || '
           and d.yy = ' || substr(pIn_nEndCalcOfDate, 1, 4);
