...
create or replace package body FMP_Update_Node_OBJ is
  ...
  begin
    ...
    FMP_log.logBegin;
    select f.f_cle, f.f_desc
      into vProduct, vDescription
      from fam f
    ...
    v_sqlStr := 'insert into  ' || vtb_tmppvt || '
     select seq_pvt.nextval pvt_em_addr,
             ...
             ''' || vDescription || ''' || decode(g.g_desc, null, '''', ''-'') || g.g_desc ||
             decode(d.d_desc, null, '''', ''-'') || d.d_desc pvt_desc,
