...
create or replace package body FMP_Attributes is
  ...
  begin
    ...
    FMP_CommonUtil.FMSP_ParseOptions(pIn_vStrOptions   => pIn_vOption,
  ...
  begin
    ...
    cSQL := 'CREATE TABLE ' || pIn_strTemporaryTableName || '( ' ||
  ...
  begin
    ...
    for i in nCountAttribute .. nCountAttribute + nMax - 49 loop
  ...
  if vAttributeValue is null then
    ...
    if pIn_nTimeSeriesNodeType = 2 then
      vSelect := vSelectHead || ' sel_cle' || vSqlSeparator || ' f_cle ' ||
                 ...
                 vSqlSeparator || vAttributeValue || vSelectEnd ||
                 ' PEXPORT from v_pexport_agg';
    else
      vSelect := vSelectHead || /*' pvt_cle ' || vSqlSeparator ||*/
                 ' f_cle ' || vSqlSeparator || ' g_cle ' || vSqlSeparator ||
                 ' d_cle' || vSqlSeparator || vAttributeValue || vSelectEnd ||
      ...
      for i in 1 .. pIn_nNumberOfAttribute loop
        ...
        if pIn_nTimeSeriesNodeType = 1 then
          ...
          cSQL := 'insert into t_PIMport_TMP x (x.nodeid,crtserie_id,num_crt_serie,seq_value) ' ||
                  ...
                  to_number(i + 48) || ' = c.val_crt_serie';
          fmsp_execsql(cSQL);
        
          cSQL := 'merge into pvtcrt pc using t_PIMport_TMP t1 ' ||
                  ' on (pc.PVT35_EM_ADDR = t1.nodeid and ' ||
                  ' t1.seq_value = ' || to_number(i + 48) ||
                  ' and pc.numero_crt_pvt = ' || to_number(i + 48) ||
                  nSeq_crtserie || ') when matched then  ' ||
        ...
        if pIn_nTimeSeriesNodeType = 2 then
          ...
          cSQL := 'merge into selcrt sc using t_PIMport_TMP t1 ' ||
