create or replace package FMP_LoadDDAData is
  ...
  procedure FMSP_ContinuationOfandData(pIn_nNodeType         in number,
                                       ...
                                       pIn_nAttributeNumber  in number,
                                       pIn_nNumMOD           in number,
                                       pIn_nNeedContinuation in number default 0, --1 return continuation of ;0 not return continuation of
...
create or replace package body FMP_LoadDDAData is
  ...
  begin
    ...
    if pIn_nNeedNotes = 1 then
      ...
      vNotesSQL  := ' left join serinote sf  on sf.bdg3_em_addr = b.bdg_em_addr and sf.nopage = 0 and sf.num_mod = '||pIn_nNumMOD||' 
                      left join serinote sd  on sd.bdg3_em_addr = b.bdg_em_addr and sd.nopage = 1000 and sf.num_mod = '||pIn_nNumMOD ;
    else
    ...
    csql := 'insert into TMP_DDABASICDATA
             ...
             PID,PVAL, PDESC,PVALUNITID,PVALUNITKEY,STID,STVAL,STDESC,TCID,TCVAL,TCDESC,poperator,stoperator,tcoperator)' || csql;
          ...
          select p.pvt_em_addr    nodeid,
           ...
           where n.id = p.pvt_em_addr
             ...
             and to_date(t.annee_deb_note || '-' || t.mois_deb_note || '-' ||
                         t.jour_deb_note,
                         'yyyy-mm-dd') between
                 to_date(pIn_vStart, 'yyyy-mm-dd') and
                 to_date(pIn_vEnd, 'yyyy-mm-dd');
      elsif pIn_nNodeType = 1 then
        --aggregate node
        open pOut_rResult for
          select s.sel_em_addr    nodeid,
