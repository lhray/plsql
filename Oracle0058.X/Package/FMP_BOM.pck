CREATE OR REPLACE PACKAGE FMP_BOM IS
  ...
  procedure FMSP_handleBOM(pIn_nType        in number, --1 Monthly ,2 Weekly,3 Dayly
...
CREATE OR REPLACE PACKAGE BODY FMP_BOM IS
  ...
  BEGIN
    ...
    IF pIn_nNodeType = 1 THEN
      ...
      IF pIn_nUpDown = 1 THEN
        ...
        --add bdg
        vStrSql := 'insert /*+ append */ into bdg(bdg_em_addr,ID_bdg,b_cle,bdg_desc)';
        vStrSql := vStrSql ||
      ...
      where  ToID is null and nmc_field=' || PIn_nmcfield;
        ...
        vstrsql  := 'insert /*+ append */ into pvt(pvt_em_addr,fam4_em_addr,geo5_em_addr,dis6_em_addr
  ...
  begin
    ...
    for i in pIn_nNumber .. pIn_nCycle loop
      /*vWhereIsnotnull := vWhereIsnotnull || ' or n.T_' || i ||
                         ' is not null';
      vWhereIsnull    := vWhereIsnull || ' and n.T_' || i || ' is  null';*/
    
      /* vstrT      := vstrT || ',nvl(d.T' || i || ',0)+nvl(sum(d1.T' || i ||
      '*f.qute),0) T' || i;*/
    ...
    IF PIn_vKey is not null THEN
      cStrSql := cStrSql || ' where Key= ''' || PIn_vKey || '''';
    END IF;
    ...
    cStrSql := cStrSql || ' group by f.ToID,d.tsid,d.version,d1.YY' ||
             ...
             UPDATE
            SET ';
    cStrSql := cStrSql || vUpdatesql; --|| ' delete ' || vWhereIsnull;
    cStrSql := cStrSql || '
