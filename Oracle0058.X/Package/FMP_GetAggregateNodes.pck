...
create or replace package body FMP_GetAggregateNodes is
  ...
  begin
    ...
    if pIn_vSequence is null then
      ...
      v_Strsql := ' select t.sel_em_addr id ,sel_cle,sel_desc' ||
                  v_strfield || '
  ...
  begin
    ...
    if v_TC_level <> 0 then
      v_sql_st_where := v_sql_st_where || ' and s.nlevel=' || v_TC_level;
      select count(*)
        into v_Datascounts
        from tmp_cdt c
      ...
      if v_Datascounts <> 0 then
        ...
        case v_Ope
          when 1 then
            v_sql_tc_connectby := ' start with s.dis_em_addr in ' ||
                                  v_Sql_Part ||
                                  ' connect by prior s.dis_em_addr=s.dis2_em_addr';
          when 2 then
    ...
    if pIn_vSequence is null then
      ...
      v_Strsql := ' select t.sel_em_addr id,t.sel_cle,t.description' ||
                  v_strfield || '
