...
create or replace package body P_PIMPORT_DIMENSION is
  ...
  begin
    ...
    v_strSQL := v_strSQL || ' using (select distinct t1.parent from ' ||
                p_strTableName || ' t1 where trim(t1.parent) is not null) t';
    --on
    ...
    if p_oOptions.bSuite then
      ...
      v_strSQL := v_strSQL || ' using (select distinct t1.suite from ' ||
                  p_strTableName || ' t1 where trim(t1.suite) is not null) t';
      --on
    ...
    if p_oOptions.bTab then
      for i in 1 .. v_nPriceCount loop
        ...
        v_strSQL := v_strSQL || ' using (select distinct t1.key_prix_' || i ||
                    ' from ' || p_strTableName || ' t1 where trim(t1.key_prix_' || i ||
                    ') is not null) t';
        --on
    ...
    v_strSQL := v_strSQL || ' using (select distinct t1.Value_Unit from ' ||
                p_strTableName || ' t1 where trim(t1.Value_Unit) is not null) t';
    --on
    ...
    for i in 1 .. v_nUoMCount loop
      ...
      v_strSQL := v_strSQL || ' using (select distinct t1.key_unite_' || i ||
                  ' from ' || p_strTableName || ' t1 where trim(t1.key_unite_' || i ||
                  ') is not null) t';
      --on 
      v_strSQL := v_strSQL || ' on (vct.id_crt = ' ||
                  p_constant.v_ProductData || ' and vct.num_crt = ' ||
    ...
    --update
    v_strSQL := v_strSQL || ' update set fam.fam0_em_addr=t.parent_addr';
    ...
    for i in 1 .. v_nPriceCount loop
      v_strSQL := v_strSQL || ', fam.prix_' || i || '=nvl(t.prix_' || i||' ,fam.prix_' || i ||')';
    end loop;
    for i in 2 .. v_nUoMCount loop
      v_strSQL := v_strSQL || ', fam.unite_' || i || '=nvl(t.unite_' || i||',fam.unite_' || i ||' )';
    end loop;
    --when not matched
    ...
    if p_oOptions.bSuite then
      ...
      --update
      v_strSQL := v_strSQL ||
                  ' update set nmc.pere_pro_nmc=nvl2(t.child_adr, t.parent_adr, null),
    ...
    --UOM
    for i in 1 .. v_nUoMCount loop
      v_strSQL := 'update rfc r set r.vct10_em_addr =(select v.vct_em_addr from  '
               || p_strTableName
               ||' t,fam f,vct v  '
               ||' where t.key_unite_'||i
               ||' is not null and f.f_cle = t.key '
               ||' and v.num_crt = '|| p_constant.NumberForUOM
               ||' and v.val = t.key_unite_'||i
               ||' and r.fam7_em_addr = f.fam_em_addr '
               ||' and r.numero_crt = '|| (p_constant.NumberForUOM+i-1)
               ||' )where exists (select 1 '
               ||' from '||p_strTableName||' t,fam f,vct v  '
               ||' where t.key_unite_'||i||' is not null '
               ||' and f.f_cle = t.key '
               ||' and v.num_crt =  '|| p_constant.NumberForUOM
               ||' and v.val = t.key_unite_'||i
               ||' and r.fam7_em_addr = f.fam_em_addr '
   ...
   fmsp_execsql(v_strSQL);
    ...
    --attribute
    for i in 1 .. p_nAttrCount loop
        v_strSQL :='update rfc r set r.vct10_em_addr = (select v.vct_em_addr '
               ||' from '||p_strTableName||' t,fam f,vct v where t.attr_'||i
               ||' is not null and f.f_cle = t.key '
               ||' and v.num_crt = '||(p_constant.BaseNumberOfAttr + i)
               ||' and v.val = t.attr_'||i
               ||' and r.fam7_em_addr = f.fam_em_addr '
               ||' and r.numero_crt = '||(p_constant.BaseNumberOfAttr + i)
               ||' )where exists (select  1 from '||p_strTableName||' t,fam f,vct v '
               ||' where t.attr_'||i
               ||' is not null and f.f_cle = t.key '
               ||' and v.num_crt = '||(p_constant.BaseNumberOfAttr + i)
               ||' and v.val = t.attr_'||i
  ...
  begin
    ...
    v_strSQL := v_strSQL ||
                ...
                p_strTableName ||
                ' a left join geo b on nvl(b.g_cle,'')FM$('')=nvl(a.parent,'')FM$('') where a.key is not null) t';
    --on
    v_strSQL := v_strSQL || ' on (geo.g_cle = t.key)';
    --when matched
    v_strSQL := v_strSQL || ' when matched then';
    --update
    v_strSQL := v_strSQL || ' update set geo.geo1_em_addr=t.parent_addr';
    case p_oOptions.nDescription
      when 0 then
        null;
      when 1 then
    ...
    if p_oOptions.bTab then
      for i in 1 .. v_nPriceCount loop
        ...
        --on
        v_strSQL := v_strSQL ||
                    ' on (geodvs.n0_dvs = 0 and geodvs.geo39_em_addr=t.geo_em_addr and geodvs.dvs38_em_addr=t.dvs_em_addr)';
        --when not matched
        v_strSQL := v_strSQL || ' when not matched then';
        --insert
        v_strSQL := v_strSQL ||
    ...
    for i in 1 .. p_nAttrCount loop
      ...
      v_strSQL := v_strSQL || ' on (rfc.ident_crt = ' ||
                  ...
                  (p_constant.BaseNumberOfAttr + i) ||
                  ' and rfc.geo8_em_addr=t.geo_em_addr and rfc.vct10_em_addr=t.vct_em_addr)';
      --when not matched
      v_strSQL := v_strSQL || ' when not matched then';
      --insert
      v_strSQL := v_strSQL ||
  ...
  begin
    ...
    v_strSQL := v_strSQL || ' where a.key is not null) t';
    --on
    v_strSQL := v_strSQL || ' on (dis.d_cle = t.key)';
    --when matched
    v_strSQL := v_strSQL || ' when matched then';
    --update
    v_strSQL := v_strSQL || ' update set dis.dis2_em_addr=t.parent_addr';
    case p_oOptions.nDescription
      when 0 then
        null;
      when 1 then
    ...
    for i in 1 .. p_nAttrCount loop
      ...
      v_strSQL := v_strSQL || ' on (rfc.ident_crt = ' ||
                  ...
                  (p_constant.BaseNumberOfAttr + i) ||
                  ' and rfc.dis9_em_addr=t.dis_em_addr and rfc.vct10_em_addr=t.vct_em_addr)';
      --when not matched
      v_strSQL := v_strSQL || ' when not matched then';
      --insert
      v_strSQL := v_strSQL ||
