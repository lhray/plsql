...
create or replace package body P_PExport is
  ...
  begin
    ...
    if p_oOptions.bDebut then
      if p_oOptions.nDateFormat between
         P_BATCHCOMMAND_DATA_TYPE.SWITCH_a_m_j and
  ...
  begin
    ...
    if ncnt = 1 then
      ...
      when matched then
        update p.enddate=t.yymmdd';
      fmsp_execsql(csql);
    end if;
  
    fmsp_updatedate(pIn_vTableName => pIn_vTableName);
  exception
    when others then
  ...
  begin
    ...
    if bEnddate then
      nFlag := 1;
      if bAMJ then
        cSql := cSql || ' enddate = endperiod||''' || vsep || '''||enddate';
      else
    ...
    if bBegindate then
      ...
      if nFlag = 1 then
        if bAMJ then
          cSql := cSql || ',begindate = beginperiod||''' || vsep ||
                  '''||begindate';
        else
