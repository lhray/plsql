create or replace package P_BATCHCOMMAND_COMMON Authid Current_User is
  ...
  procedure FMSP_ConvertDateToPeriod(pIn_vTableName     in varchar2, -- Temporary table name
                                     pIn_vYYFieldName   in varchar2,
                                     pIn_vMMFieldName   in varchar2,
                                     pIn_vDDFieldName   in varchar2,
                                     pIn_vDateFieldName in varchar2,
                                     pIn_cPeriodData    in clob, -- date and period string, eg: 2012,01,04,201201;2012,03,15,201213;
...
create or replace package body P_BATCHCOMMAND_COMMON is
    ...
    while v_nOptionsLen > 0 LOOP
      ...
      case v_strOptionKey
        ...
        when 'nd' then
          ...
          --added end
      --added begin by zhanglei 20130412 bills of supply
        when 'mtotal' then
          p_oOptions.bMtotal := true;
        when 'day' then
          p_oOptions.bDay := true;
        when 'bomtotal' then
          p_oOptions.bBomtotal := true;
        when 'old_data_drp' then
          p_oOptions.old_data_drp := 1;
        when 'ajout' then
          p_oOptions.ajout := 1;
        when 'date_ajout_delai' then
          if v_strOptionParam is null then
            raise_application_error(-20005,
                                    'Invalid date_ajout_delai value!');
          end if;
          p_oOptions.date_ajout_delai := to_number(v_strOptionParam);
          --added end
        when 'pro' then
          p_oOptions.bPro := true;
        when 'geo' then
          p_oOptions.bGeo := true;
        else
          null;
      end case;
    end loop;
  ...
  begin
    ...
    FMP_BATCHLOG.FMSP_LogDimensionAttribute(p_strTmpTableName,
                                            p_nCommandNumber,
                                            p_nTaskId);
    ...
    v_strsql := 'select count(*)  from ' || p_strTmpTableName;
    execute immediate v_strsql
      into p_nImportedSuccessCount;
  ...
  begin
    ...
    FMP_BATCHLOG.FMSP_LogDimension(p_strTmpTableName,
                                   p_nCommandNumber,
                                   p_nTaskId);
  ...
  begin
    ...
    fmp_log.FMP_SetValue(pIn_vYYFieldName);
    fmp_log.FMP_SetValue(pIn_vMMFieldName);
    fmp_log.FMP_SetValue(pIn_vDDFieldName);
    fmp_log.FMP_SetValue(pIn_vDateFieldName);
    fmp_log.FMP_SetValue(pIn_cPeriodData);
    fmp_log.LOGBEGIN;
  
    VstrOption := trim(upper(pIn_cPeriodData));
    ...
    while ilength > 0 LOOP
      ...
      if inext > 1 then
        ...
        VstrOption := trim(substr(VstrOption, inext + 1));
        ilength    := length(VstrOption);
      END IF;
    
      --',' Separated values
      inext := INSTR(vOption, ',', 1, 1);
      nYY   := trim(substr(vOption, 1, inext - 1));
    
      vOption := trim(substr(vOption, inext + 1));
      inext   := INSTR(vOption, ',', 1, 1);
      nMM     := trim(substr(vOption, 1, inext - 1));
    
      vOption := trim(substr(vOption, inext + 1));
      inext   := INSTR(vOption, ',', 1, 1);
      nDD     := trim(substr(vOption, 1, inext - 1));
    
      nPeriodData := trim(substr(vOption, inext + 1));
      VStrSql     := 'update ' || pIn_vTableName || ' set ' ||
                     pIn_vDateFieldName || ' =' || nPeriodData || ' where ' ||
                     pIn_vYYFieldName || '=' || nYY || ' and ' ||
                     pIn_vMMFieldName || '=' || nMM || ' and ' ||
                     pIn_vDDFieldName || '=' || nDD;
