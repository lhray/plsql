...
create or replace package body P_pImport is
  ...
  begin
    ...
    /*    --debug
    fmsp_execsql('CREATE TABLE TBMIDIMPORT AS SELECT * FROM '||P_TableName); 
    return ;
    --debug*/
  ...
  procedure FMSP_SaveTStoSQL(pIn_nChronology in number,
    ...
    nMaxChronologyTimePoint number;
  
    cStrSql         clob;
    vWhereIsnotnull clob := ' where 1=2 ';
  ...
  begin
    ...
    if pIn_nChronology = p_constant.Monthly then
      v_Type                  := 'M';
      nMaxChronologyTimePoint := 12;
      --2 Week
    elsif pIn_nChronology = p_constant.Weekly then
      v_Type                  := 'W';
      nMaxChronologyTimePoint := 53;
    else
      v_Type                  := 'D';
      nMaxChronologyTimePoint := 371;
    end if;
    ...
    for i in 1 .. nMaxChronologyTimePoint loop
      vWhereIsnull := vWhereIsnull || ' and d.T' || i || ' is  null';
    end loop;
    ...
    p_StrSql := p_StrSql || '
           WHEN NOT MATCHED THEN
            ...
            VALUES (seq_' || v_TBName ||
                '.nextval,n.pvtID,n.TSID,n.Version,n.YY' || v_strT || ')' ||
                vWhereIsnotnull;
    FMP_LOG.LOGDEBUG(pIn_cSqlText => p_StrSql);
...
from ' || P_TableName || ' where pvtid is null';
  ...
  begin
    ...
    if pIn_nChronology = p_constant.Monthly then
      --monthly
      v_TBName               := '_M';
      nMaxChronlogyTimePoint := 12;
    elsif pIn_nChronology = p_constant.Weekly then
      --2 Weekly
      v_TBName               := '_W';
      nMaxChronlogyTimePoint := 53;
    elsif pIn_nChronology = p_constant.Daily then
      --3 Daily
      v_TBName               := '_D';
      nMaxChronlogyTimePoint := 371;
    end if;
    ...
    for i in 1 .. nMaxChronlogyTimePoint loop
      vWhereIsnull := vWhereIsnull || ' and d.T' || i || ' is  null';
    end loop;
  ...
  BEGIN
    ...
    select m.' || VID || ',m.tsid,m.version,d.yy' || vfield ||
               ' from (select m.* from ' || vTBName || ' m join ' ||
               pIn_tableName || ' p on m.' || VID ||
               '=p.pvtID where m.tsID=' || pIn_TSType || '  and version=' ||
  ...
  BEGIN
    ...
    vStrSql := vStrSql || ' select /*+ parallel */ t.' || VID ||
               ', t.tsid,b.yy' || vfield;
    vStrSql := vStrSql || ' from (select m.* from ' || vTBName ||
               ' m join ' || pIn_tableName || ' p on m.' || VID ||
               '=p.pvtID where m.tsID=' || pIn_TSType || '  and version=' ||
               PIn_version || ' and m.YY between ' || iYYBegin || ' and ' ||
               iYYEnd || ')  t  partition by(t.' || VID ||
               ', t.tsid) right outer join (select level+' || iYYBegin ||
               '-1 yy from dual connect by level <=' || iYYEnd || '-' ||
               iYYBegin || '+1 ) b on t.yy = b.yy ';
    fmsp_execsql(vstrSql);
