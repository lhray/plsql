...
create or replace package body P_SortSequence is
  ...
  begin
    ...
    P_StrField := ',b.bdg_em_addr BDGID,b.B_cle BDG_K,b.BDg_desc BDG_D';
  ...
  begin
    ...
    cSql := '';
    if pIn_nLevel = 1 then
      nLevel := pIn_nLevel;
    elsif pIn_nLevel > 1 then
      nLevel := 1;
      vSign  := '>=';
    end if;
    if pIn_nOrder = 1 then
      ...
      if pIn_nLevel = 0 then
        ...
        cSql := 'insert into ' || pOut_vTableName ||
                ' select t.childid,t.parentid,v.nodeid,v.key,v.description,v.nodetype,t.nlevel
                    from (select s.pere_bdg parentID, s.fils_bdg childID, level nlevel
                           from supplier s
                           start with s.fils_bdg in (select bdgid from ' ||
                pIn_vTableName || ')
        ...
        cSql := 'insert into ' || pOut_vTableName ||
                ' select t.childid,t.parentid,v.nodeid,v.key,v.description,v.nodetype ,t.nlevel
                    from (select childID,parentID,nlevel from (
                                 select s.pere_bdg parentID, s.fils_bdg childID ,level nlevel,
                                    count(distinct level) over(partition by connect_by_root s.pere_bdg) countLevel
                                    from supplier s
                           where level ' || vSign || nLevel || '
                           start with s.fils_bdg in (select bdgid from ' ||
                pIn_vTableName || ')
                        connect by nocycle  s.fils_bdg = prior s.pere_bdg) where countLevel>=' ||
                pIn_nLevel || ' and nlevel<='||pIn_nLevel||') t
               left join FMV_BdgNodeID v
                on v.bdgid = t.parentID';
      end if;
    else
      ...
      if pIn_nLevel = 0 then
        ...
        cSql := 'insert into ' || pOut_vTableName ||
                ' select t.childid,t.parentid,v.nodeid,v.key,v.description,v.nodetype,t.nlevel
                    from (select s.pere_bdg childID, s.fils_bdg parentID, level nlevel
                           from supplier s
                           start with s.pere_bdg in (select bdgid from ' ||
                pIn_vTableName || ')
                        connect by nocycle prior s.fils_bdg =  s.pere_bdg) t
               left join FMV_BdgNodeID v
                on v.bdgid = t.parentID';
      else
        cSql := 'insert into ' || pOut_vTableName ||
                ' select t.childid,t.parentid,v.nodeid,v.key,v.description,v.nodetype ,t.nlevel
                    from (select childID,parentID,nlevel from (
                     select s.pere_bdg childID, s.fils_bdg parentID ,level nlevel,
                            count(distinct level) over(partition by connect_by_root s.fils_bdg) countLevel
                           from supplier s
                           where level ' || vSign || nLevel || '
                           start with s.pere_bdg in (select bdgid from ' ||
                pIn_vTableName || ')
                        connect by nocycle prior s.fils_bdg =  s.pere_bdg) where countLevel>=' ||
                pIn_nLevel || ' and nlevel<='||pIn_nLevel||') t
               left join FMV_BdgNodeID v
                on v.bdgid = t.parentID';
      end if;
    ...
    if cSql is not null then
      fmsp_execsql(pIn_cSql => cSql);
      fmp_log.LOGINFO(pIn_cSqlText => cSql);
    end if;
    fmp_log.LOGEND;
  exception
    when others then
