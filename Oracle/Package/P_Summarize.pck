...
create or replace package body P_Summarize is
  ...
  begin
    ...
    for i_Nth in 1 .. 6 loop
      begin
        ...
        if v_AggregationBasedon <> 2 then
          -- sum hist val
          ...
          if i_Nth = 1 then
            ...
            v_CurrentSemicolonIdx := instr(p_TimeSeriesNo,
                                           ';',
                                           1,
                                           i_Nth - 1);
          ...
          if v_IsAddExist = 0 and i_Nth in (1, 2, 3) then
            --when specified Total of Detail Level Hisorical Data
            --delete all data on aggregate nodes then calc from detail nodes
            fmsp_execsql(pIn_cSql => 'delete from prb_' || v_MorWorD ||
                                     ' p where p.tsid in ' ||
                                     v_TimeSeriesIDS ||
                                     ' and exists (select 1 from  prvsel l where l.prv15_em_addr=' ||
                                     P_AggregateRuleID ||
                                     ' and l.sel16_em_addr=p.selid)');
          end if;
        
          --process  UoM  Option
          if v_Unite in (0, 80, 22) then
            -- 1:Basic,80:group,22:without
            --Without: nodes values in the aggregation are stored and consolidated in product UM1 but the corresponding aggregate nodes have no unit
              ...
              when 'M' THEN
                delete from prb_m p
                 where exists (select 1
                         ...
                         where l.prv15_em_addr = P_AggregateRuleID
                           and l.sel16_em_addr = p.selid)
                   and (p.tsid = 49 or p.tsid = 59 or p.tsid = 92 or
                       p.tsid in v_TimeseriesNO);
              when 'W' THEN
                delete from prb_w p
                 where exists (select 1
                          from prvsel l
  ...
  begin
    ...
    for i in 1 .. f_ConvertTcnt(p_MorWorD) loop
      v_sql2 := v_sql2 || 'p.t' || i || '=p.t' || i || '+nvl(t.t' || i ||
                ',0)#';
    
    end loop;
    v_sql2 := substr(replace(v_sql2, '#', ','),
                     1,
                     length(replace(v_sql2, '#', ',')) - 1);
  ...
  begin
    fmp_log.LOGDEBUG(pIn_vText => 'summarize', pIn_cSqlText => p_Sql);
