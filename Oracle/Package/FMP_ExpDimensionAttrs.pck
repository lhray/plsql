create or replace package FMP_ExpDimensionAttrs is
  ...
  procedure FMSP_ExpDimensionAttrs(pIn_nCommandNumber in number,
                                   ...
                                   pIn_chSdlt         in char,
                                   pIn_nDecimalPrice  in number,
                                   pIn_nDecimalUoM    in number,
...
create or replace package body FMP_ExpDimensionAttrs is
  ...
  procedure FMSP_ExpProduct(pIn_bIsgroup       in boolean,
                            ...
                            pIn_chSdlt         in char,
                            pIn_nDecimalPrice  in number default 0,
                            pIn_nDecimalUoM    in number default 0,
                            pOut_vTmpTableName out varchar2,
                            pOut_nSqlCode      out number)
  --*****************************************************************
    ...
    --       pIn_nDecimal:add decimal to process price
  ...
  begin
    ...
    if pIn_tOptions.bSuite then
      ...
      vDecimalColumns := FMCP_FUNCTIONS.FMCF_NumberFormatSQL(pIn_vDataColumn => ' c.qute ',
                                                             pIn_nDecimal    => 3);
    
      cSQLSelect := cSQLSelect || pIn_chSdlt || '||c.f_cle||' || pIn_chSdlt || '||' ||
                    pIn_vSeperator || vDecimalColumns || '||' ||
                    pIn_vSeperator || 'chr(null)||';
    ...
    for iprix in 1 .. nPriceCnt loop
      vDecimalColumns := FMCP_FUNCTIONS.FMCF_NumberFormatSQL(pIn_vDataColumn => ' p.prix_' ||
                                                                                iprix,
                                                             pIn_nDecimal    => pIn_nDecimalPrice);
    
      cSQLSelect := cSQLSelect || '||' || pIn_vSeperator || vDecimalColumns;
    ...
    for ium in 1 .. nUMcnt loop
      if ium = 1 then
        ...
        --process 70 to 78
        vDecimalColumns := FMCP_FUNCTIONS.FMCF_NumberFormatSQL(pIn_vDataColumn => ' u.um' || ium,
                                                               pIn_nDecimal    => pIn_nDecimalUoM);
      
        cSQLSelect := cSQLSelect || '||' || pIn_vSeperator ||
                      vDecimalColumns || '||' || pIn_vSeperator ||
                      pIn_chSdlt || '||u.c' || to_char(68 + ium) || '||' ||
                      pIn_chSdlt;
      end if;
    end loop;
    cSQLLeft := cSQLLeft ||
  ...
  --*****************************************************************
    ...
    --       pIn_chSdlt:export option sdlt
    --       pIn_nDecimalPrice:decimal of price
    --       pIn_nDecimalUoM::decimal of UoM
    --       pOut_vTmpTableName:all the export datas stored in this table
    --       pOut_nSqlCode:0 successful otherwise failed
    -- Error Conditions Raised:
    --
    ...
    -- --------------------------------------------------------------
    --  V7.0        12-DEC-2012     JY.Liu      Created.
    --  V7.0        03-APR-2012     JY.Liu      add decimal of price and UoM.    
    -- **************************************************************
  ...
  begin
    ...
    Fmp_Log.FMP_SetValue(pIn_nDecimalPrice);
    Fmp_Log.FMP_SetValue(pIn_nDecimalUoM);
    ...
    case
      ...
      when pIn_nCommandNumber = 4 then
        FMSP_ExpProduct(pIn_bIsgroup       => true,
                        ...
                        pIn_vSeperator     => vSeperator,
                        pIn_chSdlt         => vSdlt,
                        pIn_nDecimalPrice  => pIn_nDecimalPrice,
                        pIn_nDecimalUom    => pIn_nDecimalUoM,
                        pOut_vTmpTableName => pOut_vTmpTableName,
                        pOut_nSqlCode      => pOut_nSqlCode);
