...
create or replace package body p_Selection is
  ...
  begin
    ...
    select count(*)
     ...
     where r.sel13_em_addr = p_SelectionID
       and rownum < 2;
    if v_cnt = 0 then
      cSql := 'insert   into rsp(rsp_em_addr,sel13_em_addr,pvt14_em_addr)
     select /*+index(p IDX_PVT_DIS6_EM_ADDR) index(p IDX_PVT_FAM4_EM_ADDR) index(p IDX_PVT_GEO5_EM_ADDR)*/  seq_rsp.nextval,' ||
              p_SelectionID || ',pvt_em_addr   ' || v_FromClause;
  ...
  procedure SPPrv_GetDetailNodeSqlClause(p_SqlClauseForDetailNode out varchar2,
    ...
    v_Sql_BetweenPart clob; --part of sql for creating the KEY scope when meeting 'between' operation
  ...
  begin
    ...
    if v_P_CondtionCount > 0 then
      v_sql_p := ' select 1 from (' || v_sql_p ||
                 ') x where x.fam_em_addr=p.fam4_em_addr ';
      v_Sql   := ' and exists ( ' || v_sql_p || ' ) ';
    end if;
    if v_ST_CondtionCount > 0 then
      v_sql_st := ' select 1 from (' || v_sql_st ||
                  ') x where x.geo_em_addr=p.geo5_em_addr ';
      v_Sql    := v_Sql || ' and exists (' || v_sql_st || ') ';
    end if;
    if v_TC_CondtionCount > 0 then
      v_sql_tc := ' select 1 from (' || v_sql_tc ||
                  ') x where x.dis_em_addr=p.dis6_em_addr ';
      v_Sql    := v_Sql || ' and exists (' || v_sql_tc || ') ';
    end if;
    if v_N_CondtionCount > 0 then
      v_sql_data := 'select 1 from (' || v_sql_data ||
                    ') x where x.pvt_em_addr=p.pvt_em_addr';
      v_Sql      := v_Sql || ' and exists  (' || v_sql_data || ') ';
    end if;
  ...
  begin
    ...
    execute immediate 'insert   into rsp(rsp_em_addr,sel13_em_addr,pvt14_em_addr)
     select /*+index(p IDX_PVT_DIS6_EM_ADDR) index(p IDX_PVT_FAM4_EM_ADDR) index(p IDX_PVT_GEO5_EM_ADDR)*/
      seq_rsp.nextval,' || p_SelectionID ||
  ...
  begin
    ...
    SP_GetDetailNodeByConditions(P_Conditions       => PIn_vConditions,
                                 ...
                                 P_Sequence         => PIn_vSequence,
                                 p_DetailNode       => pOut_DetailNode,
                                 pOut_vTabName      => vTable2,
                                 p_SqlCode          => pOut_SqlCode);
    pOut_vTabName := fmf_gettmptablename;
    cSQL          := ' create table ' || pOut_vTabName ||
                     ' as select * from ' || vTable1 ||
                     ' intersect select * from ' || vTable2;
    fmsp_execsql(pIn_cSql => cSQL);
    execute immediate 'truncate table TB_TS_DetailNodeSelCdt';
    cSQL := 'insert into TB_TS_DetailNodeSelCdt select id from ' ||
            pOut_vTabName;
    fmsp_execsql(pIn_cSql => cSQL);
    fmsp_execsql(pIn_cSql => 'drop table ' || vTable1 || ' purge');
    fmsp_execsql(pIn_cSql => 'drop table ' || vTable2 || ' purge');
    open pOut_DetailNode for 'select * from ' || pOut_vTabName || ' order by pvt_cle ';
