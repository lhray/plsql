create or replace package p_Aggregation is
  ...
  procedure SP_UpdateAggregation(p_TimeSeriesNoForUoM in varchar2,
                                 ...
                                 pIn_vFMUser          in varchar2,
                                 pIn_nMaj_Sel         in number default 3,
                                 p_SqlCode            out number);
  ...
  procedure FMSP_BuildAggregateRule_ID(pIn_nAggregateRuleID in number,
                                       ...
                                       pIn_nType            in number,
                                       pIn_nObjType         in number,
                                       pIn_vFMUser          in varchar2 default 'FM',
                                       pOut_nSqlCode        out number);
...
create or replace package body p_Aggregation is
  ...
  procedure SP_UpdateAggregation(p_TimeSeriesNoForUoM in varchar2,
                                 ...
                                 p_SqlCode            out number) as
    --*****************************************************************
    -- Description: pimport 100
    --
    -- Parameters:
    --       p_TimeSeriesNoForUoM
    --       p_TimeSeriesNoNomal
    --       p_UoMConfig
    --       p_Config
    --       pIn_vFMUser
    ...
    -- --------------------------------------------------------------
    --  V7.0        4-June-2013     L.Sang        Modify.
    -- **************************************************************

    v_SQLStmt      clob;
    v_SelRowsCnt   number :=0;
    v_PvtRowsCnt   number;
    v_PipeName     varchar2(4000);
    v_jobno        number;
  ...
  begin
    ...
    Fmp_log.FMP_SetValue(pIn_vFMUser);
    Fmp_log.FMP_SetValue(pIn_nMaj_Sel);
    Fmp_Log.LOGBEGIN;
...
--    execute immediate 'truncate table rsp';
    ...
    if pIn_nMaj_Sel = 0 or pIn_nMaj_Sel=1 or pIn_nMaj_Sel=3 then
      -- 0 is blank
      -- 1 is reRun
      -- 3 is redo and check bad agg
     execute immediate 'truncate table rsp';
    ...
    if v_PvtRowsCnt > 0 then
      v_SQLStmt := 'begin p_aggregation.spprv_updateAggregation(''' ||
                   ...
                   pIn_vFMUser || ''',:start_id, :end_id); end; ';
    ...
    if pIn_nMaj_Sel = 3 then
      -- if the value  = 3  ,need check the bad aggregate node.
    delete from sel s
     where s.sel_bud = 71
       and not exists
   ...
   /*************every type need summarize ,but tyep =2 only  run summarize   *****************************/
  ...
  procedure sp_GenerateDetailNodeOfTheRule(p_AggRuleID  in number,
    ...
    v_sql_detailnode_Norm clob := ' insert  into aggregation_detailnode(aggregationid,DetailNodeID) select ' ||
                                  p_AggRuleID ||
                                  ', pvt_em_addr
                                            from v_detailnodeattrvalue d where 1=1 ';

    v_sql_detailnode_OBJ clob := ' insert  into AGG_DETAILNODE_info(aggregationid,DetailNodeID) select ' ||
                                 p_AggRuleID ||
                                 ', pvt_em_addr
                                            from v_detailnodeattrvalue d where 1=1 ';
  ...
  begin
    ...
    fmp_log.LOGDEBUG(pIn_vModules => 'aggregation', pIn_cSqlText => v_sql,pIn_vText => '99999');
  ...
  begin
    ...
    --sel
    nSysdate := f_getcurrentoledatetime;
    insert into sel
      (sel_em_addr, sel_bud, sel_cle, user_create_sel, date_create_sel)
      select seq_sel.nextval, 71, vName, pIn_vFMUser, nSysdate
        from dual
    ...
    update sel s
       set s.effectif       =
           ...
           s.user_modify_sel = pIn_vFMUser,
           s.date_modify_sel = nSysdate;
  ...
  procedure spprv_ProduceAggNodeAndLinks(P_AggregateRuleID in number,
    ...
    vWhere                    clob;
    nSysdate                  number := f_getcurrentoledatetime;
  begin
    if pIn_nType = 0 then
    ...
    if v_isNULLRule = 0 then
      ...
      fmsp_ExecSql(v_sql_create_table_aggregate);
    else
      v_sql_create_table_aggregate := 'create table ' ||
                                      v_tablename_aggregate ||
                                      ' as select
    ...
    fmsp_ExecSql(v_sql_alter_tab_add_cols);
    ...
    fmsp_ExecSql(v_sql_text);
    v_sql_text := ' update '|| v_tablename_aggregate 
                ||' t set t.fullname = replace(substr(t.fullname, 1, 92),chr(1),'''||v_aggregationrulename||''') ' ;
    fmsp_ExecSql(v_sql_text);            
    ...
    if pIn_nType = 1 then
      v_SqlRSP_clause := 'insert  into rsp (rsp_em_addr,pvt14_em_addr,sel13_em_addr)
      ...
      for k in (select t.COLUMN_NAME
        ...
        v_sql_text := 'insert  into cdt  (cdt_em_addr, n0_cdt,  rcd_cdt, operant, n0_val_cdt, adr_cdt, sel11_em_addr)
    ...
    v_sql_text := 'insert  into  sel(sel_em_addr,sel_bud,sel_cle,sel_desc,user_create_sel,date_create_sel)
    select nodeid,71,fullname,fulldesc ,''' || pIn_vFMUser || ''',' ||
                  nsysdate || ' from ' || v_tablename_aggregate ||
                  ' t where not exists (select 1 from aggregatenode_fullid p
         where p.aggregationid = ' || P_AggregateRuleID || '
           and t.fullids = p.aggregatefullid)';
    begin
      fmp_log.LOGDEBUG(pIn_vText =>'BuildAgg' ,pIn_cSqlText => v_sql_text);
      execute immediate v_sql_text;
    exception
      when Dup_val_on_index then
        v_sql_text := 'update ' || v_tablename_aggregate ||
                      ' t set t.fullname = substr(''' ||
                      v_aggregationrulename ||
                      ''' ||''-''|| t.fullname, 1, 92) where exists (select 1 from aggregatenode_fullid s where s.aggregationid <> ' ||
                      P_AggregateRuleID || '  and t.fullname = s.name)';
        fmsp_ExecSql(v_sql_text);

        v_sql_text := 'insert  into  sel(sel_em_addr,sel_bud,sel_cle,sel_desc,user_create_sel,date_create_sel)
        select nodeid,71,fullname,fulldesc,''' ||
                      pIn_vFMUser || ''',' || nsysdate || ' from ' ||
                      v_tablename_aggregate ||
                      ' t where not exists (select 1 from aggregatenode_fullid p
         where p.aggregationid = ' ||
                      P_AggregateRuleID || '
    ...
    --fmsp_ExecSql('drop table ' || v_tablename_detail || ' purge');
    --fmsp_ExecSql('drop table ' || v_tablename_aggregate || ' purge');
    fmp_log.LOGDEBUG(pIn_vText =>v_tablename_detail||'---'||v_tablename_aggregate );
  exception
    when others then
