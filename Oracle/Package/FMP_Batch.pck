...
create or replace package body FMP_Batch is
  ...
  --*****************************************************************
   ...
   is
    ...
    v_ModifyDate number;
  begin
    v_ModifyDate := F_GetCurrentOleDateTime;
    ...
    if pIn_tSwitches.pro_n0crt = 0 AND pIn_tSwitches.pro = false then
      ...
      vSql := 'insert  into fam(fam_em_addr,id_fam,f_cle,F_Desc,user_create_fam,date_create_fam,fam0_em_addr)
      ...
      update fam f set f.date_modify_fam = v_ModifyDate where f.fam_em_addr =1;
  ...
  begin
    ...
    if pIn_tSwitches.geo then
      ...
      if pIn_tSwitches.geo_n0crt <> 0 then
        ...
        vSql := 'insert  into geo(geo_em_addr,g_cle,g_desc,user_create_geo,date_create_geo, geo1_em_addr)
          select seq_geo.nextval,' || vStrSql || ',''' ||
           ...
           where rowid =(select max(rowid) from ' ||
                ...
                vStrSql || ' = g.g_cle)';
        FMSP_execsql(pIn_cSql => vSql);
        update geo g set g.date_modify_geo  = v_ModifyDate where g.geo_em_addr = 1;
      end if;
    end if;
  ...
  begin
    ...
    if pIn_tSwitches.dis then
      ...
      if pIn_tSwitches.dis_n0crt <> 0 then
        ...
        vSql := 'insert  into dis(dis_em_addr,d_cle,d_desc,user_create_geo,date_create_dis, dis2_em_addr)
          select seq_dis.nextval,' || vStrSql || ',''' ||
           ...
           where rowid =(select max(rowid) from ' ||
                ...
                vStrSql || ' = d.d_cle)';
        FMSP_execsql(pIn_cSql => vSql);
        update dis d set d.date_modify_dis = v_ModifyDate where d.dis_em_addr = 1;
      end if;
    end if;
  ...
  begin
    ...
    if pIn_nNodeType = 1 or pIn_tSwitches.p2r then
      --detail node
      nCurrentTime := F_ConvertDateToOleDateTime(sysdate);
      cInsertPvt   := 'insert  into pvt(pvt_em_addr,pvt_cle,pvt_desc,
      ...
      cInsertBdg := 'insert  into bdg(bdg_em_addr,ID_bdg,b_cle,bdg_desc)
      ...
      cInsertBdg := cInsertBdg || ' from ' || cSql ||
                    ' where not exists (select 1 from bdg b where b.b_cle=' ||
      ...
      cCreate := ' create table ' || vTable ||
                 ...
                 ' left join sel s on s.sel_bud=71 and s.sel_cle = t.sel_cle  '||
                  --and s.sel_cle=' || cName ||
                  /**** modify BY L.Sang  ,because import table exists sel  key ****/
