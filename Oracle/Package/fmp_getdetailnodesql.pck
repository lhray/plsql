    ...
    if v_Datascounts <> 0 then
      ...
      case v_Ope
        ...
        /*  pInOut_vProductWhere := pInOut_vProductWhere ||
          ...
          fmsp_execsql('truncate table tmp_product');
          v_sql := 'insert into tmp_product (select distinct fam_em_addr from v_fam_tree  f where  f.isleaf = 1 and f.id_fam=80
                           start with f.fam_em_addr in ' ||
                                  v_Sql_Part ||
                                  ' connect by prior f.fam_em_addr=f.fam0_em_addr)';
          fmsp_execsql(v_sql);
         pInOut_vProductWhere := pInOut_vProductWhere ||'  and not exists
                   (select 1
                            from tmp_product f
                           where f.productid = p.fam_em_addr) ';
        when 3 then
    ...
    if v_Datascounts <> 0 then
      ...
      case v_Ope
        ...
        /*  pInOut_vSaleTWhere := pInOut_vSaleTWhere ||
         ...
         fmsp_execsql('truncate table tmp_sel');

         v_sql :=' insert into tmp_sel (select g.geo1_em_addr  from v_geo_tree g where g.isleaf = 1
                           start with g.geo_em_addr in ' ||
                                v_Sql_Part || '
                           connect by prior g.geo_em_addr=g.geo1_em_addr) ';
        fmsp_execsql(v_sql);
         pInOut_vSaleTWhere := pInOut_vSaleTWhere ||' and not exists (select 1 from tmp_sel where selid = s.geo_em_addr ) ';
        when 3 then
          --parse a string like this '(124,125)' to 2 string like this '124' and '125'
          select g.g_cle
            into v_F_Begin
    ...
    if v_Datascounts <> 0 then
      ...
      case v_Ope
        ...
        when 2 then
         ...
         fmsp_execsql('truncate table tmp_trade_channel');
                           --t.dis_em_addr=d.dis_em_addr and
         v_sql := ' insert into tmp_trade_channel (select d.dis_em_addr  from v_dis_tree d where d.isleaf = 1
                           start with d.dis_em_addr in ' ||
                                 v_Sql_Part || '
                           connect by prior d.dis_em_addr=d.dis2_em_addr)';
        fmsp_execsql(v_sql);
        pInOut_vTradeCWhere := pInOut_vTradeCWhere ||' and  not exists  (select 1 from tmp_trade_channel where channelid = t.dis_em_addr ) ';
        when 3 then
