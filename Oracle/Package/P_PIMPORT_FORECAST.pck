create or replace package P_PIMPORT_FORECAST Authid Current_User is
  ...
  procedure sp_DetailNode_note(p_nObjectCode                in number,
                               ...
                               P_FMUSER                     in varchar2,
                               pInOut_nImportedSuccessCount in out integer,
...
create or replace package body P_PIMPORT_FORECAST is
  ...
  begin
    ...
    if p_nObjectCode not in (1051, 1052) then
      ...
      v_strsql := 'select count(*)  from ' || v_strTableName;
      execute immediate v_strsql
  ...
  begin
    ...
    fmsp_execsql('drop table ' || v_strTableName || ' purge');
  ...
  begin
    ...
    execute immediate v_strsql
      into pInOut_nImportedSuccessCount;
      
    fmsp_execsql('drop table ' || v_strTableName || ' purge');

    fmsp_execsql('drop table ' || P_TableName || ' purge');
...
--add log
   --   Fmp_Log.logInfo(pIn_cSqlText => '5_'||v_strsqlhead || v_strsqlheader || ')' || v_strsql);
    ...
    fmsp_execsql(v_strsqlhead || v_strsqlheader || ')' || v_strsql);
    ...
    fmsp_execsql(v_strsqlheader || v_strsql);
    ...
    fmsp_execsql(v_strsqlheader || v_strsql || v_strsql_From ||
    ...
    */
    ...
    if p_nObjectCode in (52, 2074, 2078, 2082) then
      fmsp_execsql('drop table ' || p_strTableName);
