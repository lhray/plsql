...
create or replace package body FMP_ExportForecast is
  ...
  begin
    ...
    v_strSQLNOMTotal1 := '  t.TYPE_PARAM        F_Model,
                            cast(decode(n.fils_BDG,0,null,n.fils_BDG) as varchar2(100)) REFERENCE_NODE,
                            t.HORIZON           Horizon,
    ...
    */
    ...
    if instr(v_StrOption, 'MTOTAL') = 0 then
      ...
      if instr(v_StrOption, 'SEL') > 0 then
        ...
        if p_nObjectCode in (52, 2074, 2078, 2082) then
          v_strSQL := v_strSQL ||
                      ' select m.sel16_em_addr node_em_addr,n.sel_cle 
                                from prvsel m left outer join sel n
                                on m.sel16_em_addr=n.sel_em_addr
                                where n.sel_bud=71
                                and m.prv15_em_addr=' ||
    ...
    p_nSqlCode := 0;
    fmp_log.LOGEND;
    /*v_strSQL:= 'create table test as select * from '||p_strTableName;
    execute immediate v_strSQL;*/
  exception
    when others then
      p_nSqlCode := sqlcode;
