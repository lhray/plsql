...
create or replace package body P_PEXPORT_QueryTimeSeries is
  ...
  function FMF_GetSqlInsert(pIn_vTablename  in varchar2,
                            pIn_nChronology in number) return clob is
    cSQL clob;
  begin
  
    cSQL := 'insert into ' || pIn_vTablename || ' (';
    cSQL := cSQL || '   fieldNO,';
    cSQL := cSQL || '   Node_addr,';
    cSQL := cSQL || '   Chronology,';
    cSQL := cSQL || '   Time_series_id,';
    cSQL := cSQL || '   Version,';
    cSQL := cSQL || '   Annee,';
    cSQL := cSQL || '   Begin_year,';
    cSQL := cSQL || '   Begin_period,';
    cSQL := cSQL || '   End_year,';
    cSQL := cSQL || '   End_period,';
    case pIn_nChronology
  ...
  --*****************************************************************
   ...
   is
    ...
    cSqlMonthlyInsert    clob;
    cSqlWeeklyInsert     clob;
    cSqlDailyInsert      clob;
    cSqlMonthlySelect    clob;
    cSqlWeeklySelect     clob;
    cSqlDailySelect      clob;
    cSqlMonthly          clob;
    cSqlWeekly           clob;
    cSqlDaily            clob;
    bMonthlyUnionAllFlag boolean := false; -- when the 1st select appended ,the value will be true.
    bWeeklyUnionAllFlag  boolean := false;
    bDailyUnionAllFlag   boolean := false;
  begin
    pOut_nSQLCode := 0;
    ...
    if nperiods2 = nperiods3 and nperiods2 = nperiods4 and
      ...
      v_strsql := v_strsql || '   fieldNO       number,';
      ...
      for j in 0 .. nperiods2 loop
        ...
        case v_arrChronology(j)
          when P_CONSTANT.Monthly then
            if bMonthlyUnionAllFlag then
              cSqlMonthlySelect := cSqlMonthlySelect || ' union all  ' ||
                                   v_c_lob;
            else
              bMonthlyUnionAllFlag := true;
              --monthly insert into clause
              cSqlMonthlyInsert := FMF_GetSqlInsert(pIn_vTablename  => pOut_vTableName,
                                                    pIn_nChronology => P_CONSTANT.Monthly);
              cSqlMonthlySelect := v_c_lob;
            end if;
          when P_CONSTANT.Weekly then
            if bWeeklyUnionAllFlag then
              cSqlWeeklySelect := cSqlWeeklySelect || ' union all ' ||
                                  v_c_lob;
            else
              bWeeklyUnionAllFlag := true;
              --weekly insert into clause                                            
              cSqlWeeklyInsert := FMF_GetSqlInsert(pIn_vTablename  => pOut_vTableName,
                                                   pIn_nChronology => P_CONSTANT.Weekly);
            
              cSqlWeeklySelect := v_c_lob;
            end if;
          when P_CONSTANT.Daily then
            if bDailyUnionAllFlag then
              cSqlDailySelect := cSqlDailySelect || ' union all ' ||
                                 v_c_lob;
            else
              bDailyUnionAllFlag := true;
              --daily insert into clause                                            
              cSqlDailyInsert := FMF_GetSqlInsert(pIn_vTablename  => pOut_vTableName,
                                                  pIn_nChronology => P_CONSTANT.Daily);
              cSqlDailySelect := v_c_lob;
            end if;
        end case;
      end loop;
      if bMonthlyUnionAllFlag then
        cSqlMonthly := cSqlMonthlyInsert || ' ' || cSqlMonthlySelect;
        fmsp_execsql(pIn_cSql => cSqlMonthly);
      end if;
      if bWeeklyUnionAllFlag then
        cSqlWeekly := cSqlWeeklyInsert || ' ' || cSqlWeeklySelect;
        fmsp_execsql(pIn_cSql => cSqlWeekly);
      end if;
      if bDailyUnionAllFlag then
        cSqlDaily := cSqlDailyInsert || ' ' || cSqlDailySelect;
        fmsp_execsql(pIn_cSql => cSqlDaily);
      end if;
    end if;
  exception
    when others then
